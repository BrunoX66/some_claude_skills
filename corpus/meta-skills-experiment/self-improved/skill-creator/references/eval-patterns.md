...
**Good patterns:**
- Spot-check specific values verifiable from the input
...
- `"The function is implemented"` -- existence does not equal correctness

**Common additions:**
- `"The code does not contain placeholder comments like 'TODO' or 'pass'"` -- checks for stub implementations
- `"The code imports only standard library modules"` if that is a requirement
...
- `"The process completed successfully"` -- tautological if there is an output
...
For these assertions, write a script -- do not eyeball it:
